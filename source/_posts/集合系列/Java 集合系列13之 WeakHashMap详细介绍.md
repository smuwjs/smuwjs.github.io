...
2. WeakHashMap是哈希表，但是它的键是"弱键"。WeakHashMap中保护几个重要的成员变量：table, size, threshold, loadFactor, modCount, queue。
    1. **table** 是一个Entry[]数组类型，而Entry实际上就是一个单向链表。哈希表的"key-value键值对"都是存储在Entry数组中的。 
    2. **size** 是Hashtable的大小，它是Hashtable保存的键值对的数量。 
    3. **threshold** 是Hashtable的阈值，用于判断是否需要调整Hashtable的容量。threshold的值="容量*加载因子"。
    4. **loadFactor** 就是加载因子。 
    5. **modCount** 是用来实现fail-fast机制的
    6. **queue** 保存的是“已被GC清除”的“弱引用的键”。
...
public class WeakHashMap<K,V>
    ...
    public WeakHashMap(int initialCapacity, float loadFactor) {
        ...
        // 设置“WeakHashMap阈值”，当WeakHashMap中存储数据的数量达到threshold时，
        // 就需要将WeakHashMap的容量加倍。
...
WeakReference是“弱键”实现的哈希表。

**它这个“弱键”的目的就是：实现对“键值对”的动态回收。** 当“弱键”不再被使用到时，GC会回收它，WeakReference也会将“弱键”对应的键值对删除。
