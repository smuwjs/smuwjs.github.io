...
AbstractSequentialList 实现了`get(int index)、set(int index, E element)、add(int index, E element) `和` remove(int index)`这些函数。这些接口都是随机访问List的，LinkedList是双向链表；既然它继承于`AbstractSequentialList`，就相当于已经实现了`get(int index)`这些接口。
